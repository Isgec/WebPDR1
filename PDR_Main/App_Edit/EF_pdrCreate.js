...
var script_pdrCreate = {
    ...
    validate_ProjectID: function(sender) {
      ...
      this.validated_FK_PDR_Defects_ProjectID_main = true;
      this.validate_FK_PDR_Defects_ProjectID(sender,Prefix);
      },
    validate_ElementID: function(sender) {
      var Prefix = sender.id.replace('ElementID','');
      this.validated_FK_PDR_Defects_ElementID_main = true;
      this.validate_FK_PDR_Defects_ElementID(sender,Prefix);
      },
    validate_FK_PDR_Defects_ProjectID: function(o,Prefix) {
      var value = o.id;
      var ProjectID = $get(Prefix + 'ProjectID');
      if(ProjectID.value==''){
        if(this.validated_FK_PDR_Defects_ProjectID_main){
      ...
      value = value + ',' + ProjectID.value ;
        ...
        PageMethods.validate_FK_PDR_Defects_ProjectID(value, this.validated_FK_PDR_Defects_ProjectID);
      },
    validated_FK_PDR_Defects_ProjectID_main: false,
    validated_FK_PDR_Defects_ProjectID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pdrCreate.validated_FK_PDR_Defects_ProjectID_main){
    ...
    validate_FK_PDR_Defects_ElementID: function(o,Prefix) {
      var value = o.id;
      var ElementID = $get(Prefix + 'ElementID');
      if(ElementID.value==''){
        if(this.validated_FK_PDR_Defects_ElementID_main){
      ...
      value = value + ',' + ElementID.value ;
        ...
        o.style.backgroundRepeat= 'no-repeat';
        o.style.backgroundPosition = 'right';
        PageMethods.validate_FK_PDR_Defects_ElementID(value, this.validated_FK_PDR_Defects_ElementID);
      },
    validated_FK_PDR_Defects_ElementID_main: false,
    validated_FK_PDR_Defects_ElementID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pdrCreate.validated_FK_PDR_Defects_ElementID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
