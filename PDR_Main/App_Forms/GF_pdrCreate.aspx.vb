...
Partial Class GF_pdrCreate
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/PDR_Main/App_Display/DF_pdrCreate.aspx"
  ...
  End Sub
  Protected Sub GVpdrCreate_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpdrCreate.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim PDRNo As Int32 = GVpdrCreate.DataKeys(e.CommandArgument).Values("PDRNo")  
        Dim RedirectUrl As String = TBLpdrCreate.EditUrl & "?PDRNo=" & PDRNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "Deletewf".ToLower Then
      Try
        Dim PDRNo As Int32 = GVpdrCreate.DataKeys(e.CommandArgument).Values("PDRNo")  
        SIS.PDR.pdrCreate.DeleteWF(PDRNo)
        GVpdrCreate.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim PDRNo As Int32 = GVpdrCreate.DataKeys(e.CommandArgument).Values("PDRNo")  
        SIS.PDR.pdrCreate.InitiateWF(PDRNo)
        GVpdrCreate.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpdrCreate_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpdrCreate.Init
    DataClassName = "GpdrCreate"
    SetGridView = GVpdrCreate
  End Sub
